...
const CONTROLLER_ID = 'api-tryit-hurl-notebook-controller';
const NOTEBOOK_TYPE = 'api-tryit-hurl-notebook';
const CONTROLLER_LABEL = 'Hurl Runner';

/**
 * Notebook controller that executes individual Hurl request cells.
 *
 * Each cell's text is written to a temporary `.hurl` file and executed via the
 * existing `hurl-runner`. The result is rendered as Markdown in the cell output
 * area — showing status code, duration, assertions, and (when available) the
 * raw response body captured by `hurl -i`.
 */
export class HurlNotebookController {
    ...
    private async executeCell(
        ...
        try {
            ...
            // Write cell content to a temporary hurl file so the runner can execute it.
            tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'hurl-notebook-'));
    ...
    private buildOutputs(fileResult: HurlFileResult): vscode.NotebookCellOutput[] {
        ...
        if (fileResult.entries.length > 0) {
            ...
            // No entries — show error/stderr output.
    ...
    private assertionsForEntry(
        ...
        // Fall back to file-level assertions attributed to this entry by name, or to the
        // first entry when there is only one entry in the file.
