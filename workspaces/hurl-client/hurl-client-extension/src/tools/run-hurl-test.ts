...
function buildToolOutput(
    ...
    const input = {
        requests: allItems.map((item: ApiRequestItem) => ({
            ...
            headers: item.request.headers.map((h: { key: string; value: string }) => ({ key: h.key, value: h.value })),
            queryParameters: item.request.queryParameters.map((q: { key: string; value: string }) => ({ key: q.key, value: q.value })),
